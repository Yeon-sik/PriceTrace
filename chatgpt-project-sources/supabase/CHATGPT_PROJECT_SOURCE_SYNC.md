...
1. `src/domain/receipt.ts`와 `docs/templates/RECEIPT_V2_TEMPLATE.json`, merchant-only 입력이면 `src/domain/merchant-profile.ts`와 `docs/templates/MERCHANT_PROFILE_V1_TEMPLATE.json`을 실제 계약에 맞춘다.
2. 필요하면 `docs/contracts/VERIFIED_RECEIPT_INGESTION_V2.md`, `docs/contracts/MERCHANT_PROFILE_V1.md`, `scripts/validate-private-receipts.ts`, `scripts/private-receipt-source.ts`, `GOAL.md`, `chatgpt-project-sources/PROJECT_INSTRUCTIONS.md`와 `PASTE_TO_PROJECT_SETTINGS.md`를 갱신한다.
...
- merchant-profile.v1 필드·nullable·사용자 검증·merchant identity 후보 등록 경계 변경
