...
`merchant-profile.v1`은 영수증 없이 사용자가 “가게 + 상호명”으로 시작한 **검증 전 판매처 source fact 초안**이다. PriceTrace UUID, SKU, 표준 상품, 브랜드, 카탈로그 연결은 포함하지 않는다.
...
ChatGPT Project는 이 draft만 만든다. OCR App 또는 관리 UI가 사용자의 확인을 마친 뒤에만 다음 RPC의 `p_merchant`로 전달할 수 있다.
