...
as $$
  ...
  eligible_mappings as (
    ...
    inner join unique_codes as source
      on source.source_product_code = mapping.source_product_code
  ),
  eligible_catalog as (
    select
  ...
  select
    ...
    catalog.content_unit,
    catalog.package_count,
    coupang.listed_price_krw as coupang_listed_price_krw,
    coupang.quantity as coupang_quantity,
    coupang.product_url as coupang_product_url,
    coupang.observed_at as coupang_observed_at
  from eligible_mappings as mapping
  inner join eligible_catalog as catalog
    on catalog.catalog_product_id = mapping.catalog_product_id
  inner join public.standard_products as standard
    on standard.id = catalog.standard_product_id
  left join latest_coupang as coupang
    on coupang.standard_product_id = standard.id
