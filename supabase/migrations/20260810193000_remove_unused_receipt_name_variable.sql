...
declare
  ...
  v_definition text;
  v_updated text;
  v_unused_declaration text := $fragment$  v_normalized_receipt_name text := regexp_replace(coalesce(p_receipt_product_name, ''), '[[:space:]]+', '', 'g');
$fragment$;
begin
  select pg_catalog.pg_get_functiondef(v_signature)
  into v_definition;

  if pg_catalog.strpos(v_definition, v_unused_declaration) = 0
  then
    raise exception 'Unexpected strict core receipt-name declaration.';
  end if;

  v_updated := pg_catalog.replace(v_definition, v_unused_declaration, '');
