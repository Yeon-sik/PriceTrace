alter table public.catalog_products
  add column specification_status text not null default 'verified';

alter table public.catalog_products
  add constraint catalog_products_specification_status_check
  check (specification_status in ('verified', 'placeholder'));

alter table public.catalog_products
  add constraint catalog_products_placeholder_specification_check
  check (
    specification_status = 'verified'
    or (
      content_amount = 1
      and content_unit = 'each'
      and package_count = 1
      and reference_unit = 100
    )
  );

comment on column public.catalog_products.specification_status is
  'placeholder means 1 each x 1 is a non-commercial sentinel and must not be used in unit-price calculations.';
