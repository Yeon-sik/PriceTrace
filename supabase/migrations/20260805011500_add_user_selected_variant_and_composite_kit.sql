-- Add item-specific user selection and heterogeneous razor-kit support without
-- relaxing automatic name discovery. This migration also restores the one-source-label
-- boundary at the RPC itself. Prior migrations remain immutable.

create or replace function public.is_verified_explicit_user_selected_frozen_pair(
  p_target jsonb,
  p_receipt_name_raw text,
  p_official_name_raw text,
  p_receipt_source_id text,
  p_official_source_id text,
  p_specification_text_raw text
)
returns boolean
language sql
stable
...
as $function$
  select
    p_target -> 'sameChannelNameRule' ->> 'outcome' = 'apply_verified_name_equivalence'
    and p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'method'
      = 'explicit_user_selected_frozen_pair_v1'
    and p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'scope'
      = 'frozen_receipt_official_pair_only'
    and p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'selectedReceiptSourceId'
      = p_receipt_source_id
    and p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'selectedOfficialSourceId'
      = p_official_source_id
    and p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'selectedNormalizedReceiptName'
      = pg_catalog.regexp_replace(p_receipt_name_raw, '[[:space:]]+', '', 'g')
    and p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'selectedNormalizedOfficialName'
      = pg_catalog.regexp_replace(p_official_name_raw, '[[:space:]]+', '', 'g')
    and p_target -> 'userSelectedOfficialVariant' ->> 'scope'
      = 'frozen_receipt_official_pair_only'
    and p_target -> 'userSelectedOfficialVariant' ->> 'selectedReceiptSourceId'
      = p_receipt_source_id
    and p_target -> 'userSelectedOfficialVariant' ->> 'selectedOfficialSourceId'
      = p_official_source_id
    and p_target -> 'userSelectedOfficialVariant' ->> 'selectedSpecificationTextRaw'
      = p_specification_text_raw
    and p_target -> 'userSelectedOfficialVariant' ->> 'selectionSourceRef'
      = p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'userSelectionSourceRef'
    and p_target -> 'userSelectedOfficialVariant' ->> 'selectionContentHash'
      = p_target -> 'sameChannelNameRule' -> 'verifiedEquivalence' ->> 'userSelectionContentHash'
    and p_target -> 'userSelectedOfficialVariant' ->> 'selectionContentHash'
      ~ '^sha256:[a-f0-9]{64}$'
    and p_target -> 'review' ->> 'verdict' = 'approve'
    and p_target -> 'review' ->> 'reviewerAgent' = 'pricetrace_independent_reviewer'
    and p_target -> 'review' ->> 'evidenceQuality' = 'sufficient'
    and p_target -> 'review' -> 'conflicts' = '[]'::jsonb
    and exists (
      select 1 from pg_catalog.jsonb_array_elements(p_target -> 'evidence') as evidence(item)
...
revoke all on function public.is_verified_explicit_user_selected_frozen_pair(
  jsonb, text, text, text, text, text
) from public, anon, authenticated;

-- Extend the already-applied link-only RPC with audited structured official
-- specifications while preserving its signature, approval gate, and writes.
-- This file is intentionally append-only; prior migrations remain immutable.

create or replace function public.approve_and_register_standard_product_link_only_v1(
...
declare
  ...
  v_user_selection jsonb;
  v_kit_components jsonb;
  v_expected_attributes jsonb := '{}'::jsonb;
  ...
  v_parsed_unit text;
  v_total_unit text;
  v_parsed_package_count integer;
  v_parsed_total_amount numeric;
  v_parse_rule text;
  v_package_count_basis text;
  v_matched_fragment text;
...
begin
  ...
  v_user_selection := v_target -> 'userSelectedOfficialVariant';
  ...
  if coalesce(v_target ->> 'executionMode', '') <> 'link_only_v1'
    ...
    or cardinality(p_source_labels) <> 1
  ...
  if v_user_selection is not null and (
    pg_catalog.jsonb_typeof(v_user_selection) <> 'object'
    or v_user_selection ->> 'scope' <> 'frozen_receipt_official_pair_only'
    or v_user_selection ->> 'selectedReceiptSourceId' <> v_receipt_source_id
    or v_user_selection ->> 'selectedOfficialSourceId' <> v_official_source_id
    or v_user_selection ->> 'selectedSpecificationTextRaw'
      <> v_official ->> 'specificationTextRaw'
    or coalesce(v_user_selection ->> 'selectionSourceRef', '') = ''
    or coalesce(v_user_selection ->> 'selectionContentHash', '')
      !~ '^sha256:[a-f0-9]{64}$'
    or coalesce(v_user_selection ->> 'selectedAt', '') = ''
  ) then
    raise exception 'The item-specific user selection does not match the frozen pair.'
      using errcode = '23514';
  end if;
  ...
  if not (
    ...
    or public.is_verified_explicit_user_selected_frozen_pair(
      v_target,
      v_receipt ->> 'sourceNameRaw',
      v_official ->> 'sourceNameRaw',
      v_receipt_source_id,
  ...
  then
    v_expected_attributes := pg_catalog.jsonb_build_object('apparelSize', p_apparel_size);
    ...
    if v_specification_check ->> 'kind' = 'composite_kit' then
      v_specification_match := pg_catalog.regexp_match(
        btrim(v_official ->> 'specificationTextRaw'),
        '^면도기\s*(\d+)\s*면도날\s*(\d+)$'
      );
      if v_specification_match is null then
        raise exception 'The official composite kit specification is invalid.'
          using errcode = '23514';
      end if;
      v_kit_components := pg_catalog.jsonb_build_array(
        pg_catalog.jsonb_build_object(
          'componentType', 'razor_handle',
          'quantity', v_specification_match[1]::integer,
          'unit', 'each'
        ),
        pg_catalog.jsonb_build_object(
          'componentType', 'razor_blade',
          'quantity', v_specification_match[2]::integer,
          'unit', 'each'
        )
      );
      v_expected_attributes := pg_catalog.jsonb_build_object(
        'kitComponents', v_kit_components
      );
      if v_specification_match[1]::integer <= 0
        or v_specification_match[2]::integer <= 0
        or v_specification_check ->> 'parseRule' <> 'razor_handle_blade_kit_v1'
        or (v_specification_check ->> 'parsedSellableContentAmount')::numeric <> 1
        or v_specification_check ->> 'parsedSellableContentUnit' <> 'each'
        or (v_specification_check ->> 'parsedSellablePackageCount')::integer <> 1
        or v_specification_check -> 'kitComponents' <> v_kit_components
        or v_identity -> 'kitComponents' <> v_kit_components
        or p_content_amount <> 1
        or p_content_unit <> 'each'
        or p_package_count <> 1
        or p_reference_unit <> 100
        or p_specification <> v_official ->> 'specificationTextRaw'
        or not (v_rule -> 'importedOfficialFields' ? 'kitComponents')
      then
        raise exception 'The composite kit does not match the fingerprinted target.'
          using errcode = '23514';
      end if;
    elsif v_specification_check ->> 'kind' = 'wiper_blade_fitment' then
      v_specification_match := pg_catalog.regexp_match(
        btrim(v_official ->> 'specificationTextRaw'),
        '^(\d{3})\s*mm$',
        'i'
      ...
      if v_specification_match is null
        or coalesce(v_official ->> 'sourceNameRaw', '') not like '%와이퍼%'
        or v_specification_match[1]::integer not between 250 and 800
      then
        raise exception 'The official wiper blade fitment is invalid.'
          using errcode = '23514';
      end if;
      v_expected_attributes := pg_catalog.jsonb_build_object(
        'wiperBladeFitment',
        pg_catalog.jsonb_build_object('lengthMm', v_specification_match[1]::integer)
      );
      if v_specification_check -> 'parsedWiperBladeFitment'
          <> v_expected_attributes -> 'wiperBladeFitment'
        or v_identity -> 'wiperBladeFitment'
          <> v_expected_attributes -> 'wiperBladeFitment'
        or p_content_amount <> 1
        or p_content_unit <> 'each'
        or p_package_count <> 1
        or p_reference_unit <> 100
        or p_specification <> v_official ->> 'specificationTextRaw'
        or not (v_rule -> 'importedOfficialFields' ? 'wiperBladeFitment')
      then
        raise exception 'The wiper blade fitment does not match the fingerprinted target.'
          using errcode = '23514';
      end if;
    elsif v_specification_check ->> 'kind' = 'structured_content' then
      v_specification_match := pg_catalog.regexp_match(
        btrim(v_official ->> 'specificationTextRaw'),
        '^(\d+(?:\.\d+)?)\s*(kg|g|ml)\s*[xX×*]\s*(\d+)\s*개(?:입)?\s*/\s*(\d+(?:\.\d+)?)\s*(kg|g|ml)$',
        'i'
      );
      if v_specification_match is not null then
        v_parsed_amount := v_specification_match[1]::numeric;
        v_parsed_unit := lower(v_specification_match[2]);
        v_parsed_package_count := v_specification_match[3]::integer;
        v_parsed_total_amount := v_specification_match[4]::numeric;
        v_total_unit := lower(v_specification_match[5]);
        if v_parsed_unit = 'kg' then
          v_parsed_amount := v_parsed_amount * 1000;
          v_parsed_unit := 'g';
        end if;
        if v_total_unit = 'kg' then
          v_parsed_total_amount := v_parsed_total_amount * 1000;
          v_total_unit := 'g';
        end if;
        if v_total_unit = v_parsed_unit
          and v_parsed_total_amount = v_parsed_amount * v_parsed_package_count
        then
          v_parse_rule := 'per_item_times_count_with_total_v1';
          v_package_count_basis := 'explicit_specification';
        else
          raise exception 'The stated structured total does not match the per-item amount and count.'
            using errcode = '23514';
        end if;
      else
        v_specification_match := pg_catalog.regexp_match(
        btrim(v_official ->> 'specificationTextRaw'),
        '^(\d+(?:\.\d+)?)\s*(kg|g|ml)\s*\/\s*(\d+)\s*매입$',
        'i'
        );
      end if;
      if v_parse_rule is null and v_specification_match is not null then
        v_parsed_total_amount := v_specification_match[1]::numeric;
        v_parsed_unit := lower(v_specification_match[2]);
        v_parsed_package_count := v_specification_match[3]::integer;
        if v_parsed_unit = 'kg' then
          v_parsed_total_amount := v_parsed_total_amount * 1000;
        ...
        v_parsed_amount := v_parsed_total_amount / v_parsed_package_count;
        v_parse_rule := 'total_amount_per_count_v1';
        v_package_count_basis := 'explicit_specification';
      else
        v_specification_match := pg_catalog.regexp_match(
          btrim(v_official ->> 'specificationTextRaw'),
          '^(\d+(?:\.\d+)?)\s*(kg|g|ml)\s*[xX×*]\s*(\d+)\s*개입$',
          'i'
        );
        if v_specification_match is not null then
          v_parsed_amount := v_specification_match[1]::numeric;
          ...
          v_parse_rule := 'per_item_times_count_v1';
          v_package_count_basis := 'explicit_specification';
        else
          v_specification_match := pg_catalog.regexp_match(
            btrim(v_official ->> 'specificationTextRaw'),
          ...
          if v_specification_match is not null then
            v_parsed_amount := 1;
            v_parsed_unit := 'each';
            v_parsed_package_count := v_specification_match[1]::integer;
            v_parse_rule := 'count_only_v1';
            v_package_count_basis := 'explicit_specification';
          else
            v_specification_match := pg_catalog.regexp_match(
              btrim(v_official ->> 'specificationTextRaw'),
            ...
            if v_specification_match is not null then
              select count(*), min(match_value[2])
              into v_fragment_count, v_matched_fragment
              from pg_catalog.regexp_matches(
                v_official ->> 'sourceNameRaw',
                '(^|[^0-9.])(' || replace(v_specification_match[1], '.', '\.')
                  || '\s*(kg|g|ml))([^[:alnum:].]|$)',
                'gi'
              ...
              if v_fragment_count = 1 then
                v_parsed_amount := v_specification_match[1]::numeric;
                v_parsed_unit := lower(
                  (pg_catalog.regexp_match(v_matched_fragment, '(kg|g|ml)$', 'i'))[1]
                );
                if v_parsed_unit = 'kg' then
                  v_parsed_amount := v_parsed_amount * 1000;
                  v_parsed_unit := 'g';
                end if;
                v_parsed_package_count := 1;
                v_parse_rule := 'numeric_spec_unit_from_official_name_v1';
                v_package_count_basis := 'default_one_absent_count';
              end if;
            end if;
          end if;
        end if;
      ...
      if v_parse_rule is null
        or coalesce(v_parsed_amount, 0) <= 0
        or coalesce(v_parsed_package_count, 0) <= 0
        or v_specification_check ->> 'parseRule' <> v_parse_rule
        or v_specification_check ->> 'packageCountBasis' <> v_package_count_basis
        ...
        or (v_specification_check ->> 'parsedPackageCount')::integer <> v_parsed_package_count
        or v_parsed_amount <> p_content_amount
        or v_parsed_unit <> p_content_unit
        or v_parsed_package_count <> p_package_count
        or p_specification <> v_official ->> 'specificationTextRaw'
        or not (v_rule -> 'importedOfficialFields' ? 'contentAmount')
        or not (v_rule -> 'importedOfficialFields' ? 'contentUnit')
        or (
          v_package_count_basis = 'explicit_specification'
          and not (v_rule -> 'importedOfficialFields' ? 'packageCount')
        )
        or (
          v_package_count_basis = 'default_one_absent_count'
          and (
            p_package_count <> 1
            or (v_rule -> 'importedOfficialFields' ? 'packageCount')
          )
        )
        or (
          v_parse_rule in ('total_amount_per_count_v1', 'per_item_times_count_with_total_v1')
          and (
            (v_specification_check ->> 'parsedTotalContentAmount')::numeric
              <> v_parsed_total_amount
            or v_parsed_total_amount <> v_parsed_amount * v_parsed_package_count
          )
        )
        or (
          v_parse_rule not in ('total_amount_per_count_v1', 'per_item_times_count_with_total_v1')
          and v_specification_check ? 'parsedTotalContentAmount'
        )
        or (
          v_parse_rule = 'numeric_spec_unit_from_official_name_v1'
          and v_specification_check ->> 'matchedOfficialNameFragment' <> v_matched_fragment
        )
        or (
          v_parse_rule <> 'numeric_spec_unit_from_official_name_v1'
          and v_specification_check ? 'matchedOfficialNameFragment'
        )
      then
        raise exception 'The structured official content specification does not match the target.'
          using errcode = '23514';
      end if;
    else
      v_specification_match := pg_catalog.regexp_match(
  ...
  if coalesce(v_identity ->> 'brand', '') <> btrim(p_brand_name)
    ...
    or coalesce(v_identity -> 'kitComponents', 'null'::jsonb) is distinct from
      coalesce(v_expected_attributes -> 'kitComponents', 'null'::jsonb)
    or coalesce(v_identity -> 'wiperBladeFitment', 'null'::jsonb) is distinct from
      coalesce(v_expected_attributes -> 'wiperBladeFitment', 'null'::jsonb)
  ...
  if v_catalog_product_id is null then
    ...
    if not found or v_existing_catalog.status <> 'active'
      ...
      or coalesce(v_existing_catalog.attributes, '{}'::jsonb) <> v_expected_attributes
    then
...
comment on function public.approve_and_register_standard_product_link_only_v1(
  ...
  'Atomically approves and links one frozen receipt and official listing without creating a Coupang observation; supports item-specific user selection, apparel size, and typed composite razor kits.';
