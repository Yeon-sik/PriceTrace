...
test("음식점 Brand에서 메뉴 가격과 정확 Nutrition 연결을 확인한다", async ({ page }) => {
  await page.route("**/rest/v1/rpc/get_restaurant_menu_read_v1", async (route) => {
    await route.fulfill({
        ...
        restaurants: [{
          ...
          menus: [{
            ...
            observations: [{
              ...
              restaurantSourceId: "44444444-4444-4444-8444-444444444444",
              locationLabel: "강남점",
              unitPriceKrw: 9000,
              quantity: 1,
              totalPriceKrw: 9000,
  ...
  await expect(page.getByRole("button", { name: /음식점 둘러보기/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /상품 둘러보기/ })).toBeVisible();
  await page.getByRole("button", { name: /음식점 둘러보기/ }).click();
