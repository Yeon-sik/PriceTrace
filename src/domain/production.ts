...
export function receiptFingerprint(receipt:Receipt){const itemSignature=receipt.items.map(item=>`${item.storeProductCode}:${item.unitPriceKrw}:${item.purchasedQuantity}:${item.totalPriceKrw}`).sort().join("|");return `${receipt.storeLabel}|${receipt.purchasedAt}|${receipt.transactionNumber}|${receipt.totalPriceKrw}|${itemSignature}`;}
