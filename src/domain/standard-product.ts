...
export function summarizeStandardProducts(products: { id: string; name: string }[], variantsByProduct: Record<string, StandardProductVariant[]>): StandardProductSummary[] {
  return products.flatMap((product) => {
    ...
    return lowest ? [{ id: product.id, name: product.name, lowestUnitPriceKrw: lowest.normalized.pricePerReferenceUnitKrw, unitLabel: lowest.normalized.referenceLabel, lowestVariant: lowest.variant, variants }] : [];
