...
export function createUniversalReceipt(store = "Test Store", date = "2026-07-22", transactionId = "TX-1", amountMinor = 1_000, sourceProductCode = "P1") {
  return ReceiptJsonSchema.parse({
    ...
    document: { id: `receipt:${store}:${transactionId}`, type: "receipt", status: "final", issued_on: date, issued_at: `${date}T00:00:00+09:00`, currency: "KRW", fulfillment: { type: "unknown", evidence: "unknown" }, source: { capture_method: "manual_entry", original_document_id: transactionId, source_images: [], transcription_status: "user_verified", notes: [] } },
