...
export function AuthPanel({ onChange, onOpen, modal = false, onClose }: Props) {
  ...
  if (!modal) return <button type="button" className={styles.loginButton} onClick={onOpen}>관리자 접근 <span aria-hidden="true">↗</span></button>;
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (result.error) setMessage(result.error.message);
      ...
      if (result.data.user?.app_metadata?.role !== "admin") {
        ...
        setMessage("관리자 이메일로만 접근할 수 있습니다.");
        return;
      }
  ...
  return <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose?.()}><section className={styles.authModal} role="dialog" aria-modal="true" aria-labelledby="auth-title"><button type="button" className={styles.closeButton} onClick={onClose} aria-label="관리자 접근 창 닫기">×</button><p className={styles.kicker}>ADMINISTRATION</p><h2 id="auth-title">관리자 접근</h2><p>관리자 계정으로만 접근할 수 있습니다.</p><form onSubmit={submit}><label>이메일<input type="email" autoComplete="email" required value={email} onChange={(event) => setEmail(event.target.value)} autoFocus /></label><label>비밀번호<input type="password" autoComplete="current-password" required minLength={6} value={password} onChange={(event) => setPassword(event.target.value)} /></label><button className={styles.submitButton} type="submit">관리자 접근</button></form>{message && <p className={styles.authMessage} role="status">{message}</p>}</section></div>;
}
