...
export function CartQuantityModal({ product, value, error, onChange, onClose, onConfirm }: { product: CartProduct; value: string; error: string; onChange: (value: string) => void; onClose: () => void; onConfirm: () => void }) {
  ...
  return <div className={styles.modalBackdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={`${styles.authModal} ${styles.cartModal}`} role="dialog" aria-modal="true" aria-labelledby="cart-quantity-title"><button type="button" className={styles.closeButton} onClick={onClose} aria-label="장바구니 담기 창 닫기">×</button><p className={styles.kicker}>ADD TO CART</p><h2 id="cart-quantity-title">몇 개 담을까요?</h2><p><b>{product.productName}</b><br /><small>{product.storeLabel} · {priceLabel}</small></p><label className={styles.cartQuantityLabel} htmlFor="cart-quantity">추가할 수량<input id="cart-quantity" type="number" inputMode="numeric" min="1" step="1" value={value} onChange={(event) => onChange(event.target.value)} autoFocus /></label>{error && <p className={styles.cartModalError} role="alert">{error}</p>}<button type="button" className={styles.submitButton} onClick={onConfirm}>장바구니에 담기</button></section></div>;
}
