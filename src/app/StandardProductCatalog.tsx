...
type MarketObservation = { catalog_product_id: string; seller_name: string; listed_price_krw: number; shipping_fee_krw: number; observed_at: string; verification_status: string };

export function StandardProductCatalog() {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      client.from("market_price_observations").select("catalog_product_id,seller_name,listed_price_krw,shipping_fee_krw,observed_at,verification_status").eq("verification_status", "verified"),
  ...
  }, [standards, variants, receiptObservations, marketObservations]);

  if (!client || summaries.length === 0) return null;
  return <section className={styles.standardCatalog} aria-labelledby="standard-catalog-title"><div className={styles.browserHead}><div><p className={styles.kicker}>STANDARD PRODUCT PRICE</p><h2 id="standard-catalog-title">표준 상품 최저 단위가격</h2><p>같은 표준 상품에 연결된 모든 판매 규격을 100g·100ml·1개 기준으로 환산해 비교합니다.</p></div></div><div className={styles.standardCatalogGrid}>{summaries.map((summary) => <article key={summary.id}><span>표준 상품</span><h3>{summary.name}</h3><strong>{summary.unitLabel} {formatKrw(summary.lowestUnitPriceKrw)}</strong><p>최저: {summary.lowestVariant.sellerName} · {summary.lowestVariant.listingName}</p><small>{summary.variants.length}개 판매 기록</small><details><summary>판매 규격 보기</summary>{summary.variants.map((variant, index) => { const normalized = normalizeMarketPrice({ sellerName: variant.sellerName, listedPriceKrw: variant.listedPriceKrw, shippingFeeKrw: variant.shippingFeeKrw ?? 0, minimumOrderQuantity: 1, observedAt: variant.observedAt, verificationStatus: "verified" }, variant); return <p key={`${variant.id}:${variant.sellerName}:${index}`}>{variant.sellerName} · {variant.listingName} · {formatKrw(normalized.effectivePriceKrw)} · {normalized.referenceLabel} {formatKrw(normalized.pricePerReferenceUnitKrw)}</p>; })}</details></article>)}</div></section>;
}
