...
export function OfficialProductWorkspace({ candidates, onSelect, revision }: { candidates: OfficialProductCandidate[]; onSelect: (candidate: OfficialProductCandidate) => void; revision: number }) {
  ...
  const uniqueCandidates = useMemo(() => [...new Map(candidates.map((candidate) => [candidate.sourceProductCode, candidate])).values()], [candidates]);
  const states = useMemo<CandidateState[]>(() => uniqueCandidates.map((candidate) => {
    const manualRecord = manual[candidate.sourceProductCode];
  ...
  async function search(candidate: OfficialProductCandidate) {
    setSearching(candidate.sourceProductCode);
    setSearchMessages((current) => ({ ...current, [candidate.sourceProductCode]: "공식 웹 후보를 찾는 중입니다…" }));
    const response = await discoveryRepository.search(candidate);
    if (response.status === "unavailable") {
      setSearchMessages((current) => ({ ...current, [candidate.sourceProductCode]: response.message }));
    } else {
      setSearches((current) => ({ ...current, [candidate.sourceProductCode]: response.results }));
      setSearchMessages((current) => ({ ...current, [candidate.sourceProductCode]: response.results.length ? "검색 결과는 후보입니다. 브랜드·용량·구성을 확인한 뒤 연결하세요." : "공식 상품 후보를 찾지 못했습니다. 수동 검색을 사용하세요." }));
    }
    setSearching(null);
  }
  ...
  return <section className={styles.browser}>
    ...
    <section className={styles.officialSection}><h2>자동 연결된 공식 상품</h2>{matched.length ? <div className={styles.officialGrid}>{matched.map((state) => <OfficialCard candidate={state.candidate} record={state.record!} reason={state.reason} onSelect={onSelect} key={state.candidate.sourceProductCode} />)}</div> : <p>검증된 공식 상품 연결이 없습니다.</p>}</section>
    <section className={styles.officialSection}><h2>공식 상품 탐색 대기열</h2><p className={styles.manualHint}>자동 연결되지 않은 품목입니다. 로그인 후 자동 탐색을 실행하면 서버에서 공식 웹 후보를 찾아 표시합니다.</p><div className={styles.manualQueue}>{unmatched.map((state) => <article key={state.candidate.sourceProductCode}><div><strong>{state.candidate.productName}</strong><small>{state.candidate.storeLabel} · 판매처 코드 {state.candidate.sourceProductCode}</small><small>{state.reason}</small>{searchMessages[state.candidate.sourceProductCode] && <small className={styles.discoveryMessage}>{searchMessages[state.candidate.sourceProductCode]}</small>}{searches[state.candidate.sourceProductCode]?.map((result) => <div className={styles.discoveryResult} key={result.officialUrl}><b>{result.officialName}</b><small>{result.sourceName}</small>{result.description && <p>{result.description}</p>}<a href={result.officialUrl} target="_blank" rel="noreferrer">후보 열기</a><button onClick={() => onSelect({ ...state.candidate, productName: result.officialName })}>이 후보 연결</button></div>)}</div><div className={styles.queueActions}><button onClick={() => void search(state.candidate)} disabled={searching === state.candidate.sourceProductCode}>{searching === state.candidate.sourceProductCode ? "탐색 중" : "자동 탐색"}</button><a href={officialSearchUrl(state.candidate)} target="_blank" rel="noreferrer">수동 검색</a><button onClick={() => onSelect(state.candidate)}>직접 연결</button></div></article>)}</div></section>
  </section>;
}
