...
import { findSimilarBrandNames, normalizeBrandLabel, prepareBrandRegistration } from "@/domain/brand";
import {
...
function StandardProductConnectionModal({ directMode = false, candidate, legacy, brands, standards, variants, standardImages, approvalProposal, onClose, onSaved, onApproved }: { directMode?: boolean; candidate: OfficialProductCandidate; legacy?: { officialName: string; officialUrl: string; imageUrl?: string }; brands: Brand[]; standards: StandardProduct[]; variants: Variant[]; standardImages: Map<string, StandardProductImageRecord>; approvalProposal?: ReviewedLinkProposal; onClose: () => void; onSaved: (notice?: string) => void; onApproved?: () => void }) {
  ...
  const similarBrands = useMemo(() => findSimilarBrandNames(brands, brandName), [brands, brandName]);
  const normalizedBrandName = normalizeBrandLabel(brandName).toLocaleLowerCase("ko-KR");
            ...
            {similarBrands.length > 0 && <div className={styles.brandSuggestions} aria-live="polite">
              ...
              <div className={styles.brandSuggestionsList}>
                {similarBrands.map((brand) => {
                  const isExact = normalizeBrandLabel(brand.canonical_name).toLocaleLowerCase("ko-KR") === normalizedBrandName;
                  return <button type="button" key={brand.id} onClick={() => setBrandName(brand.canonical_name)}>
                    {brand.canonical_name}{isExact && <small>현재 입력과 일치</small>}
                  </button>;
                })}
              </div>
            </div>}
