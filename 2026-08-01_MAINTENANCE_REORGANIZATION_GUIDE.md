...
지금 해야 할 단 하나는 **현재 기능 브랜치의 작업을 먼저 확정하고, 정리 작업을 별도 브랜치로 격리하는 것**이다.

현재 `feat/standard-product-brands`에는 수정 22개와 미추적 16개, 총 38개 경로가 있다. 이 상태에서 정리를 시작하면 신규 브랜드·승인·표준 상품 등록 작업과 오래된 코드 정리를 구분하기 어렵다. 따라서 현재 작업을 검토·완료한 뒤 `feat/maintenance-reorganization` 같은 별도 브랜치에서 아래 순서대로 진행한다.

1. 권위 문서의 제품 범위 충돌을 먼저 결정한다.
2. 정적 도달성이 없고 대체 경로가 확인된 파일만 삭제한다.
3. 큰 UI 컴포넌트를 한 번에 하나씩 분리한다.
4. 공개 데이터 생성 파이프라인과 자동화 스크립트를 분리한다.
5. 문서의 소유권과 보존 정책을 정리한다.
6. 각 단계마다 같은 검증 묶음을 통과시킨다.

핵심 병목은 파일 수 자체가 아니다. **UI가 데이터 조회·도메인 계산·쓰기·모달 상태를 동시에 소유하는 것**, **로컬 권위 문서와 현재 제품 범위가 충돌하는 것**, **삭제 가능한 생성물과 보존해야 하는 근거 데이터가 같은 루트에서 보이는 것**이 실제 유지보수 비용의 원인이다.
