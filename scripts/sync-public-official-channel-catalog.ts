...
function buildPublicCatalog(bundle: SourceBundle): PublicOfficialChannelCatalog {
  ...
  const catalog = {
    ...
    listings: bundle.listing_snapshots.map((listing) => {
...
async function syncPublicCatalog() {
  ...
  const catalog = buildPublicCatalog(bundle);
