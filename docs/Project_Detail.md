...
| --- | --- |
| 문서 상태 | Active |
| 적용 범위 | 제품 코드 `5d6b8e1`; 운영 자동화 기준 `f86ec4b` |
| 최종 갱신 | 2026-07-26T18:57:30+09:00 |
| 최종 검증 기준 | `f86ec4b`; GitHub Actions·GitHub Pages·Notion 미러의 2026-07-26 실행 근거 |
...
- 공개 영수증의 검증과 도메인 변환, private 원본의 수동 공개 반영
...
- 정적 GitHub Pages UI에는 원본 이미지·원본 JSON·거래/결제/승인 정보·OCR 원문·service role·외부 API 비밀값을 넣지 않습니다. 검수 메모는 금지 값과 로컬 경로 검사를 통과한 경우에만 공개합니다.
- 공개 기본 데이터는 strict allowlist로 생성한 영수증별 공개 JSON, 해당 파일 목록 인덱스, 영수증 품목 연결 관측이며, 원본 영수증은 로컬 경로에서만 읽습니다.
...
| --- | --- | --- | --- | --- | --- |
| 공개 영수증 JSON 로드와 상품 탐색 | 구현 완료 | 저장소 검증 | 2026-07-27 기능 dirty tree | `PublicReceiptRepository`, 영수증별 JSON·인덱스·관측 연결 test, 공개 데이터 검사 | 배포 UI의 기능별 브라우저 smoke test 미실행 |
...
| private 원본의 수동 공개 반영 | 구현 완료 | 저장소 검증 | 2026-07-27 기능 dirty tree | `validate:private-receipts`, `sync:public-receipts`, 공개 영수증·인덱스·관측 연결 검사 | 원본의 정확성과 공개 정책 변경은 사람 검토 필요 |
| GitHub Pages 정적 배포 | 구현 완료 | 실환경 검증 | `f86ec4b` + 2026-07-26 GitHub Pages | Actions [run 30198781229](https://github.com/Yeon-sik/PriceTrace/actions/runs/30198781229) build·deploy 성공 | 배포 UI의 기능별 브라우저·모바일 smoke test 미실행 |
| 정책 기반 Notion 문서 게시 | 구현 완료 | 실환경 검증 | `f86ec4b` + 2026-07-26 GitHub Actions/Notion | `main` 병합 자동 발행 [run 30198781222](https://github.com/Yeon-sik/PriceTrace/actions/runs/30198781222) 성공; Intro·Detail source commit·fingerprint 재조회 | 정기 드리프트 감지는 미구현 |
...
private-data/receipt_YYYY-MM-DD_NNN.json
    └→ manual validate + sync command → public receipt JSON per file ─┐
                              → receipt index JSON ───────────┤
                              → linked observation JSON ──────┤
                                                         ├→ ProductBrowser / MarketBrowser / CartPage
                                                         └→ PriceTrendModal

public receipt files + index + observation JSON → strict Zod validation → revision/link 검사
                                  → public receipts + product listings
...
- **상황**: 원본 영수증에는 거래번호·결제·승인 정보, OCR 원문, 원본 이미지 경로 등 공개하면 안 되는 값이 있습니다. 판매처명·주소·전화번호·사업자등록번호와 구매 사실은 투명한 공개 정보로 유지해야 합니다.
- **제약**: 정적 배포는 로컬 파일이나 비밀값을 숨겨 주지 않습니다.
- **검토한 대안**: 실제 원본을 데모로 사용, 모든 데이터를 원격 DB에 올리기.
- **선택**: `data/public/receipts/YYYY-MM-DD_NNN.json`을 영수증 건당 하나씩 두고, `data/public/receipts/index.v1.json`에는 파일명과 개별 revision만 둡니다. `data/public/product-observations.v3.json`의 모든 관측은 `receiptId + receiptItemId`와 index revision으로 연결합니다. 앱은 항상 이 공개 데이터만 읽으며, private 원본은 수동 validate·sync의 입력으로만 사용합니다.
- **근거**: 공개 재현성과 원본 보호를 동시에 유지하며, private 파일이 없는 정적 환경에서도 실제 판매처·영수증·상품 관계를 검증할 수 있습니다.
- **비용과 위험**: 공개 판매처 정보와 정확한 발행일은 Git 이력과 정적 번들에 남습니다. 공개 범위를 바꾸려면 policy version 갱신과 기존 이력 검토가 필요합니다.
...
| GitHub Pages | 정적 UI 배포 | 비밀값 없음 | 원격 기능 장애와 UI 가용성을 분리 | main build·deploy 후 URL HTTP 확인 | 2026-07-26 `f86ec4b`, run `30198781229` |
| Notion API | Intro/Detail 읽기 전용 미러 | `notion-production` Environment Secret | 두 페이지 사전 GET, 부분 실패 집계, 원본 Git 링크 유지 | 요청 30초 timeout, 429·5xx·network 최대 4회 | 2026-07-26 `f86ec4b`, run `30198781222`, 두 페이지 재조회 |

## 9. 데이터 보호와 보안

| 경계 | 정책 | 검증 방법 |
| --- | --- | --- |
| 원본 영수증·이미지 | `private-data/`에만 두고 Git·공개 번들·앱 런타임에서 제외 | `.gitignore`, 수동 validate·sync 경로 |
| 공개 영수증 | 판매처명·주소·전화번호·사업자등록번호·발행일·품목·수량·금액을 Git 추적 | strict Zod allowlist, 공개 영수증 회귀 테스트 |
| 공개 금지 필드 | 거래번호, 결제·승인 정보, 고객 식별정보, OCR 원문, 원본 이미지 경로·파일명 제외 | 금지 키·값·경로 검사, projection 누출 회귀 테스트, `check:public-receipts` |
| 영수증-관측 연결 | 관측마다 `receiptId`, `receiptItemId`, 공개 영수증 index revision 저장 | 전체 연결 수·파일명·필드 일치 검사 |
...
| 공개 데이터 검사 | Zod + 연결 검사 | 영수증별 JSON allowlist, 인덱스·금지 키·revision, 영수증-관측 링크 | 영수증 4건·관측 223건 통과 |
| 단위/저장소 테스트 | Vitest | 영수증, 공개 projection, 상품 탐색, 카탈로그, 장바구니, 정산 불변식 | 로컬 19개 파일·57개 테스트 통과 |
| E2E | Playwright | 공개 영수증 상품 탐색·판매처 상세·장바구니·모바일 | Chromium 3개 시나리오 통과 후 runner 미종료로 명령 timeout |
...
| GitHub Pages 운영 | GitHub Actions/HTTP | static export 배포와 URL 가용성 | `f86ec4b` build·deploy 성공 |
| Supabase 운영 | Supabase | 권한·비밀값·원격 장애 흐름 | 미검증 |
| 기존 문서 동기화 | GitHub Actions/Notion | Intro·Detail 두 페이지 교체 | 2026-07-25 사용자 확인 |
| 정책 기반 문서 동기화 | GitHub Actions/Notion | `manual` 또는 `on-main-push` 정책, 사전검증 뒤 두 페이지 교체와 응답 검증 | `f86ec4b` `main` 병합에서 승인 대기 없이 자동 sync·재조회 검증 |

### 검증 이력

| 일시 | 기준 커밋 | 명령/환경 | 결과 | 근거 위치 | 미검증 항목 |
| --- | --- | --- | --- | --- | --- |
| 2026-07-27 | `feat/public-receipt-links` dirty tree | `npm.cmd run check:public-receipts` | exit 0, 영수증별 JSON 4건·연결 관측 223건·금지 키 없음·index revision/link 검증 통과 | 현재 로컬 실행 로그 | 실제 Pages 배포 |
| 2026-07-27 | 동일 | `npm.cmd run lint` / `npm.cmd run typecheck` / `npm.cmd run test` / `npm.cmd run build` | 모두 exit 0, Vitest 19 files / 57 tests, static export 성공 | 현재 로컬 실행 로그 | clean clone·배포 환경 |
| 2026-07-27 | 동일 | `npm.cmd run test:e2e` | Chromium 3개 시나리오 통과 후 runner 종료 단계에서 180초 timeout·exit 124 | 현재 로컬 실행 로그 | runner clean exit, 실제 Pages |
| 2026-07-27 | 동일 | project docs validator / Notion render-only dry run | 0 errors·0 warnings / 두 문서 render-only 검증 성공 | 현재 로컬 실행 로그 | Notion 쓰기·배포 문서 반영 |
...
| 2026-07-26 | 동일 | 이전 private server 미실행 + `npm.cmd run test:e2e` | 시나리오 1건 1.3초 성공 후 runner 미종료, 180초 timeout·exit 124 | 현재 로컬 실행 로그 | clean exit 원인, 모바일 실기기 |
...
| 2026-07-26 | `f86ec4b` | 자동 발행 [run 30198781222](https://github.com/Yeon-sik/PriceTrace/actions/runs/30198781222) / Pages [run 30198781229](https://github.com/Yeon-sik/PriceTrace/actions/runs/30198781229) | 수동 실행·승인 대기 없이 Intro 5,938자·Detail 16,421자 sync 성공; 두 페이지 source commit·fingerprint 재조회; Pages 배포 성공 | GitHub Actions 로그, Notion connector 조회, Pages 실행 | 정기 드리프트·기능별 브라우저 smoke test |
...
- 첫 `on-main-push` 자동 발행은 `f86ec4b`의 [run 30198781222](https://github.com/Yeon-sik/PriceTrace/actions/runs/30198781222)에서 별도 승인 없이 성공했고, 두 미러의 source commit과 fingerprint를 재조회했습니다.
- GitHub Pages는 같은 커밋의 [run 30198781229](https://github.com/Yeon-sik/PriceTrace/actions/runs/30198781229)에서 build·deploy가 성공했습니다.
